...
- [x] **MP-1 — repo skeleton + release pipeline** · [#2](https://github.com/dsj1984/mandrel-platform/issues/2) · no deps · **M**. `config/`, `scripts/`, `templates/runbooks/`, `.github/{workflows,actions}` trees; `package.json` exports map; `release.yml` (tag → npm publish); SHA-pinned actions; `.nvmrc` 24.x + pnpm 11.5.x canonical baseline.
- [x] **MP-2 — setup-toolchain composite action** · [#3](https://github.com/dsj1984/mandrel-platform/issues/3) · `blocked_by` #2 · **M**. pnpm-from-`packageManager` + Node-from-`.nvmrc` + `--frozen-lockfile`, `cache` boolean input. Replaces the three drifting per-repo composites. (§3 #2, §4.3)
- [x] **MP-3 — reusable `pr-quality` workflow** · [#4](https://github.com/dsj1984/mandrel-platform/issues/4) · `blocked_by` #3 · **L**. `workflow_call`; tiers + runner/shard inputs; single aggregator required-check; per-job `timeout-minutes`; Turbo remote cache; Playwright browser cache. (§3 #1/#5, §4.3, §5 Medium caching)
- [x] **MP-4 — reusable `deploy-cloudflare` workflow** · [#5](https://github.com/dsj1984/mandrel-platform/issues/5) · `blocked_by` #3 · **L**. `workflow_call` with `isolation-audit`, `check-env`, pre-migration snapshot, boot-smoke, auto-rollback. (§3 #3, §5 High deploy/migration)
- [x] **MP-5 — unconditional CodeQL workflow** · [#6](https://github.com/dsj1984/mandrel-platform/issues/6) · `blocked_by` #2 · **S**. `javascript-typescript`, not visibility-gated → runs on private repos. (§3 #9, §5 High no-SAST)
- [x] **MP-6 — config package (tsconfig/biome + CVE-gate)** · [#7](https://github.com/dsj1984/mandrel-platform/issues/7) · `blocked_by` #2 · **M**. npm-published base config + stricter `audit-check.mjs`. (§3 #8, §4.4)
- [x] **MP-7 — shared Renovate preset** · [#8](https://github.com/dsj1984/mandrel-platform/issues/8) · `blocked_by` #2 · **S**. `github>dsj1984/mandrel-platform`; nvm-lockstep + wrangler `compatibility_date` rules. (§3 #7, §4.1)
- [x] **MP-8 — main-protection contract + required-context lint** · [#9](https://github.com/dsj1984/mandrel-platform/issues/9) · `blocked_by` #4 · **M**. Single required aggregator + `check-required-contexts.mjs`. (§3 #6, §4.7)
- [x] **MP-9 — centralize common runbooks + docs-staleness lint** · [#10](https://github.com/dsj1984/mandrel-platform/issues/10) · `blocked_by` #2 · **M**. Canonical common runbooks in `docs/runbooks/` (rollback/deploy-promotion/observability/secret-rotation/dependency-update/backup-restore/branch-protection + new incident-response/SLO), **referenced** by projects; per-project `docs/environments.md` and project-specific docs stay local (classification in §6.1); + docs-staleness lint. (§3 #10, §4.7)

**Consumer track — High-severity hardening** (each bullet = one Story instantiated in all three repos):

- [x] **H1 — Deploy safety net** · domio [#1532](https://github.com/dsj1984/domio/issues/1532), athportal [#2003](https://github.com/dsj1984/athportal/issues/2003), swarm-os [#109](https://github.com/Beestera/swarm-os/issues/109) · no deps · **M–L**.
...
- [x] **H2 — Close the no-op security gates** · domio [#1533](https://github.com/dsj1984/domio/issues/1533), athportal [#2004](https://github.com/dsj1984/athportal/issues/2004), swarm-os [#110](https://github.com/Beestera/swarm-os/issues/110) · no deps · **S–M**.
...
- [x] **X2 — Adopt deploy-cloudflare + config pkg + renovate preset** · domio [#1536](https://github.com/dsj1984/domio/issues/1536), athportal [#2007](https://github.com/dsj1984/athportal/issues/2007), swarm-os [#113](https://github.com/Beestera/swarm-os/issues/113) · `blocked_by` mp **#5,#7,#8,#9,#10**, X1 · **L**.
  ...
  - **Done (all three CLOSED / `agent::done`):** every consumer cut over to `deploy-cloudflare.yml@mandrel-platform-v0.9.0` (`a606ad7`) — callers verified present in each repo. Unblocking the deploy adoption ran a long platform-gap chain (#41 → #45/#46 → #49 → #51 → #55 → #56, releases v0.4.0–v0.9.0). Moving the build consumer-side to freeze the secret allowlist is the follow-on **build-split** (mp #56, shipped v0.9.0); per-repo migrations tracked separately (domio #1545, athportal #2014, swarm-os #123).

- [x] **X3 — Adopt cross-repo workflow portability lint** · domio [#1544](https://github.com/dsj1984/domio/issues/1544), athportal [#2013](https://github.com/dsj1984/athportal/issues/2013), swarm-os [#121](https://github.com/Beestera/swarm-os/issues/121) · `blocked_by` mp **v0.3.0** (script source); pairs with **X2** · **S**.
  ...
  - **Done (all three CLOSED / `agent::done`):** `check-workflow-portability.mjs` present and wired into CI in all three repos.

### 7.7 Wave E — Platform hardening, observability & docs

- [x] **F1 — Hardening + observability + docs** · domio [#1537](https://github.com/dsj1984/domio/issues/1537), athportal [#2008](https://github.com/dsj1984/athportal/issues/2008), swarm-os [#114](https://github.com/Beestera/swarm-os/issues/114) · `blocked_by` X2, mp **#10 (MP-9)** · **M**.
  - **Done (all three CLOSED / `agent::done`):** security headers shipped (swarm-os `apps/web/src/middleware.ts` + `apps/api/src/app.ts`; athportal Hono `secureHeaders()`/Astro CSP); `analytics_engine_datasets` + `logpush` present in the wrangler configs (athportal `apps/api/wrangler.toml`, swarm-os `apps/web/wrangler.jsonc` + `apps/api/wrangler.toml`); runbook references + `.gitignore`/husky hardening per story.
...
All 27 original Stories exist (§7.1) and carry native `blocked_by` edges, plus 3 follow-on **X3** Stories (cross-repo portability lint, added post-audit from the mandrel-platform #24→#32 chain). Tick each as it is **delivered**; brackets are sequential, items within a bracket parallelize.
...
6. [x] After `mandrel-platform` #5/#7/#8/#9/#10 released **and** a repo's X1 delivered → X2 (domio #1536, athportal #2007, swarm-os #113) **+ bundled X3** (domio #1544, athportal #2013, swarm-os #121). **Delivered — all six CLOSED / `agent::done`.** All consumers pin `deploy-cloudflare.yml@mandrel-platform-v0.9.0` (`a606ad7`) and ship `check-workflow-portability.mjs`. (Deploy adoption required the platform-gap chain #41→#56, releases v0.4.0–v0.9.0; the build-split consumer migrations are tracked separately — domio #1545, athportal #2014, swarm-os #123.)
7. [x] After a repo's X2 delivered → F1 (domio #1537, athportal #2008, swarm-os #114). **Delivered — all three CLOSED / `agent::done`** (security headers + observability + docs hardening landed). **All 7 delivery-sequence steps are now complete — the original audit roadmap is fully delivered.**

**Post-audit addendum — deploy-cloudflare hardening track (delivered after the original 27+3).** The X2 deploy adoption surfaced a chain of shared-workflow gaps fixed iteratively in `mandrel-platform` (releases **v0.4.0 → v0.9.0**): boot-smoke URL (#41), Turso/build/secret command seams (#45/#46), an invalid gitleaks-action pin (#49), `gh-environment` for Environment-scoped secrets (#51), the build-secret bridge (#55), and the **build-split** (#56) that moves the build consumer-side so build secrets never cross the reusable-workflow boundary. A **live cross-repo smoke** (#38, repo `mandrel-platform-smoke`) now calls both reusable workflows as a real consumer and **gates every release** via `release-please`'s `await-smoke`. Consumer build-split migrations are delivered (domio #1545, athportal #2014, swarm-os #123). **One platform step remains (not yet filed):** remove the now-unused build-secret allowlist (`SENTRY_*`, `CLERK_*`, `PUBLIC_SENTRY_DSN`, `SITE_URL`) from `deploy-cloudflare.yml` — freezing it at `{CLOUDFLARE_*, TURSO_*}` — and drop the corresponding assertions from the smoke's `build-command` paths.
...
- **Release-gate the cutover.** Do not start Wave C/D (`X1`/`X2`) until the `mandrel-platform` units they call are merged **and** released with a pinnable SHA/tag (§6.5); back-fill the `@<sha>` placeholders in each X1/X2 body at that point. **Wave C gate is now open: use `mandrel-platform-v0.3.1` (`185f28b`) — the first cross-repo-verified release.**
