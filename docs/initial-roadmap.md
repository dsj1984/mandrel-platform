...
**Post-audit addendum — deploy-cloudflare hardening track (delivered after the original 27+3, now COMPLETE).** The X2 deploy adoption surfaced a chain of shared-workflow gaps fixed iteratively in `mandrel-platform` (releases **v0.4.0 → v0.10.0**): boot-smoke URL (#41), Turso/build/secret command seams (#45/#46), an invalid gitleaks-action pin (#49), `gh-environment` for Environment-scoped secrets (#51), the build-secret bridge (#55), the **build-split** (#56) that moves the build consumer-side so build secrets never cross the reusable-workflow boundary, and the **allowlist-removal capstone** (#61) that froze the shared deploy workflow's secret surface at `{CLOUDFLARE_*, TURSO_*}` — ending the per-consumer build-secret-bounce class for good. A **live cross-repo smoke** (#38, repo `mandrel-platform-smoke`) calls both reusable workflows as a real consumer and **gates every release** via `release-please`'s `await-smoke` (5 consecutive gated releases, v0.6.0–v0.10.0). Consumer build-split migrations are delivered (domio #1545, athportal #2014, swarm-os #123). **Track complete — nothing outstanding.**
