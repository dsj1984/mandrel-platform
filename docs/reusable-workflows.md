...
| `setup`                 | string | `'toolchain'`                                    | How the job provisions Node. `'toolchain'` runs the shared `setup-toolchain` composite (pnpm from `packageManager`, Node from `.nvmrc`, frozen `pnpm install`). `'node'` provisions Node from `.nvmrc` alone and installs **nothing** — pick it when the scanned repo is not a pnpm app, since an npm repo on the default fails at `actions/setup-node` ("Dependencies lock file is not found … `pnpm-lock.yaml`") before the scan runs. The `'node'` path requires a committed `.nvmrc` and ignores `toolchain-cache` / `pnpm-dest` / `packages-read-token`. |
